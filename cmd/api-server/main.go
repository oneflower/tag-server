...
func init() {
	// 初始化 mysql
	mysqlDB = sqlx.MustOpen("mysql", "root:password@tcp(localhost:3306)/tags?parseTime=True&loc=Local&multiStatements=true&charset=utf8mb4")
...
func OnSearchTag(c *gin.Context) {
	...
	if err != nil {
		...
		log.Printf("*** SearchTagsFromESErr: %s", tags)
		c.JSON(http.StatusInternalServerError, gin.H{
			"status":  http.StatusInternalServerError,
